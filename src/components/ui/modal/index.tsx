...
interface ModalProps {
  ...
  isCloseWhenclickingBackground?: boolean
}
...
export const Modal: React.FC<ModalProps> = ({
  ...
  isCloseWhenclickingBackground = true, // Default to true for backwards compatibility
  isFullscreen = false,
  position = 'center',
}) => {
  ...
  return (
    <div
      className={`modal fixed inset-0 z-999 flex items-${position} justify-center overflow-y-auto`}
    >
      {!isFullscreen && (
        <div
          className='fixed inset-0 h-full w-full bg-gray-400/50 backdrop-blur-[32px]'
          onClick={() => {
            if (isCloseWhenclickingBackground) {
              onClose()
            }
          }}
        ></div>
