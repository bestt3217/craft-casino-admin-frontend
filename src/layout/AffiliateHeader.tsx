...
import { getSelfReferralCode } from '@/api/affiliate'
...
const AffiliateHeader: React.FC = () => {
  ...
  const fetchReferralCode = useCallback(async () => {
    const code = await getSelfReferralCode()
  ...
  const renderApplicationMenuToggle = () => (
    <button
      ...
      <div className='flex grow flex-col items-center justify-between lg:flex-row lg:px-6'>
        ...
        >
          <div className='hidden items-center gap-4 lg:flex'>
            <Link
              href='/affiliate/friends'
              className='hover:text-brand-500 dark:hover:text-brand-400 text-sm font-medium text-gray-700 dark:text-gray-300'
              onClick={(e) => {
                e.preventDefault()
